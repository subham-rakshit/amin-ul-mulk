...
const WebsitePopupSettings = ({
  ...
  }, [isSubmitting, activeTab, getAllWebsitePopupFieldsData]);
  ...
  // Handle Footer Image State
  const onChangeFooterImage = (id) => {
    setFormData((prev) => ({
      ...prev,
      footer_image: id,
    }));
      ...
      {activeTab === "en" ? (
        <form onSubmit={handleWebsitePopupDataSubmit} className="py-4 px-5">
          ...
          <NormalBrowseFileInputFiled
            userId={userId}
            isFetching={isFetching}
            labelText="Popup Image"
            fieldId="footer_image"
            allFilesResponse={allFilesResponse}
            searchValue={searchValue}
            selectedFileType={selectedFileType}
            onChangeImageFunction={onChangeFooterImage}
            selectedFileId={formData?.footer_image || ""}
            adminRole={adminRole}

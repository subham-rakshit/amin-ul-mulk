...
const LinkWidgetOne = ({ userId, languagesResponse }) => {
  ...
  // Fetch data
  const getLinkOneFieldsData = useCallback(async () => {
    setIsFetching(true);
    ...
    try {
      ...
      console.error("Failed to fetch link one widget settings data:", error);
      useErrorToast("Failed to fetch link one widget settings.");
    } finally {
      setIsFetching(false);
    }
  }, [activeTab]);
  ...
  useEffect(() => {
    if (!isSubmitting) {
      getLinkOneFieldsData();
    }
  }, [isSubmitting, activeTab, getLinkOneFieldsData]);
  ...
  // Handle Form Submit
  const handleLinkOneDataSubmit = async (e) => {
    e.preventDefault();
    ...
    try {
      ...
      console.error("Failed to update link one widget settings:", error);
      useErrorToast("Failed to update link one widget settings.");
    } finally {
  ...
  return (
    ...
    >
      ...
      {activeTab === "en" ? (
        <form onSubmit={handleLinkOneDataSubmit} className="py-4 px-5">
          {/* Title */}
          <NormalTextInputField
            isFetching={isFetching}
            labelText="Title"
            fieldId="link-one-title"
            fieldName="link_one_title"
            placeholderText="Title"
            inputValue={formData?.link_one_title || ""}
          ...
          <NormalRepeatableInputField
            isFetching={isFetching}
            labelText="Links"
            fieldId="link-widget-one-list"
            translateField={true}
            labelFieldList={formData.link_one_label}
            valueFieldList={formData.link_one_value}
            labelFieldName="link_one_label"
            valueFieldName="link_one_value"
        ...
        <TranslationNormalForm
          fields={[
            {
              labelText: "Title",
              fieldId: `link-one-title-lang-${activeTab}`,
              fieldName: "link_one_title",
              placeholderText: "Title",
              inputValue: "",
            ...
            {
              labelText: "Links",
              fieldId: `link-widget-one-list-lang-${activeTab}`,
              fieldName: ["link_one_label"],
              labelInputPlaceholder: "Label",
              inputValue: [[]],
              isTextEditorType: false,

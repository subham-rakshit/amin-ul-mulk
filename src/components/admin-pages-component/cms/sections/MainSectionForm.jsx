...
const MainSectionForm = ({
  ...
  const handleSwitchChange = (keyName) => {
    setFormData((prev) => ({
      ...prev,
      [keyName]: prev[keyName] === "0" ? "1" : "0",
    }));
  };
  ...
  // Handle Add Multi Value Repeater
  const addMultiValueRepeaterChange = (keyName, structure = {}) => {
    setFormData((prev) => {
      const field = Array.isArray(prev[keyName]) ? prev[keyName] : [];
      return {
        ...prev,
        [keyName]: [...field, structure],
      };
    });
  };

  // Handle Remove Multi Value Repeater
  const removeMultiValueRepeaterChange = (keyName, index) => {
    setFormData((prev) => {
      const field = Array.isArray(prev[keyName]) ? prev[keyName] : [];

      if (index < 0 || index >= field.length) return prev;

      const updatedField = field.filter((_, i) => i !== index);

      return {
        ...prev,
        [keyName]: updatedField,
      };
    });
  };

  // Handle Update Multi Value Repeater
  const updateMultiValueRepeaterChange = (keyName, index, valueKey, value) => {
    setFormData((prev) => {
      const field = Array.isArray(prev[keyName]) ? prev[keyName] : [];
        ...
        <div className="mt-10">
          {isFetching ? (
            ...
            <button
              ...
              className={`min-w-[180px] ${bgColor} ${hoverBgColor} ${textColor} dark:text-light-weight-800 hover:text-white transition-300 px-5 py-2 rounded-md text-[14px] font-poppins-rg group flex items-center justify-center gap-2 ${isSubmitting ? "cursor-not-allowed" : ""}`}
            >
              {isSubmitting ? (
                <>

...
export async function GET(request, context) {
  ...
  try {
    ...
    const authCheck = await checkAuthorization({
      targetId: userId,
      requiredPermissions: [PERMISSIONS.CASE_PACKAGE.VIEW_ALL_CASE_PACKAGES],
    });
    ...
    const query = {
      ...(searchQuery && {
        $or: [{ case_package_name: { $regex: searchQuery, $options: "i" } }],
      }),
      ...(status && {
    ...
    const casePackageLists = await CasePackageModel.find(query)
    ...
    const paginationTotalArticles =
      await CasePackageModel.countDocuments(query).exec();
    ...
    return successResponse({
      status: 200,
      packages: casePackageLists,
      paginationData,
    });
  } catch (error) {
    console.log(
      `Error in getting all case packages SERVER: ${error.stack || error}`
    );
    return errorResponse({

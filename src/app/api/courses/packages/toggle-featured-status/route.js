...
export async function PATCH(request) {
  ...
  try {
    ...
    // Check Number of Featured packages must be <= 3
    const existingFeaturedPackagesCount = await TrainingCoursePackageModel.find(
      {
        courseId: existingCoursePackageDetails.courseId,
        isFeatured: true,
        isActive: true,
      }
    )
      .countDocuments()
      .exec();
    if (existingFeaturedPackagesCount >= 3) {
      return errorResponse({
        message:
          "Package feature limit exceed. Max 3 featured items can be done.",
        status: 400,
      });
    }

...
export async function GET(request, context) {
  ...
  try {
    ...
    const authCheck = await checkAuthorization({
      targetId: userId,
      requiredPermissions: [PERMISSIONS.SERVICE.VIEW_ALL_SERVICES],
    });
    ...
    const query = {
      ...(searchQuery && {
        $or: [
          ...
          { card_short_description: { $regex: searchQuery, $options: "i" } },
          { card_long_description: { $regex: searchQuery, $options: "i" } },
    ...
    const serviceLists = await ServiceModel.find(query)
      .populate({
        path: "banner_image",
        model: FilesModel,
      ...
      .select(
        "_id service_name slug banner_image is_active is_featured updatedAt"
      )
      .sort({ createdAt: -1 })
    ...
    // Pagination
    const paginationTotalServices =
      await ServiceModel.countDocuments(query).exec();

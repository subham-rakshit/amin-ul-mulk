...
export async function PATCH(request, context) {
  ...
  try {
    ...
    const updatedNewsArticle = await AllNewsArticleModel.findByIdAndUpdate(
      ...
      {
        $set: {
          is_featured: !existingNewsArticleDetails.is_featured,
        },
      },
      { new: true }
    ).exec();
    ...
    if (updatedNewsArticle.is_featured && !updatedNewsArticle.is_active) {
    ...
    return successResponse({
      message: updatedNewsArticle.is_featured

...
export async function DELETE(request) {
  ...
  try {
    ...
    // NOTE Store the default image file id in all news-blog post where the deleted image file is used
    if (deletedFile.fileType.startsWith("image/")) {
      // Update for blogs
      const allBlogPosts = await AllNewsArticleModel.find({
      ...
      for (const blog of allBlogPosts) {
        ...
        if (Object.keys(updates).length > 0) {
          await AllNewsArticleModel.updateOne(
            { _id: blog._id },
            { $set: updates }
          );
        }

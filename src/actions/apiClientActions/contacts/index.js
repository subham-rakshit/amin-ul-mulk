...
export const getAllContacts = async (
  ...
  const keyName = `${cacheKeyNames.CONTACTS}-${search || "search"}-${page || 1}-${pageSize || 9}-${status || "status"}-${featured || "featured"}-${type || "type"}`;
...
export const getAllContactQueries = async (
  ...
  // Create a unique key for the cache
  const keyName = `${cacheKeyNames.CONTACT_QUERIES}-${search || "search"}-${page || 1}-${pageSize || 9}-${name || "name"}-${email || "email"}-${number || "number"}`;
  ...
  if (parsedValue) {
    ...
    return {
      ...
      totalQueries: parsedValue?.totalQueries || [],
      paginationData: parsedValue?.paginationData || {},
    };
  }

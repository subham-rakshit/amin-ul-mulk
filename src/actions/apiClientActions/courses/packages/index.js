...
export const createNewCoursePackage = async (data, userId) => {
  try {
    ...
    if (response.data.success && response.status === 201) {
      // Revalidate the all course_packages and fe_course_details related cache
      await Promise.all([
        deletePatternCache(`${cacheKeyNames.COURSE_PACKAGES}-*`),
        deletePatternCache(`${cacheKeyNames.FE_COURSE_DETAILS}-*`),
      ]);
      console.log(
        `createNewCoursePackage: ${cacheKeyNames.COURSE_PACKAGES} ${cacheKeyNames.FE_COURSE_DETAILS} Cache deleted 🗑️`
      );

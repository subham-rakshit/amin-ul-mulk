...
export const createNewPageCMS = async (data, userId) => {
  try {
    const response = await axios.post(
      `${process.env.NEXT_PUBLIC_DOMAIN_URL}${API_ENDPOINTS.CREATE_NEW_PAGE_CMS}/${userId}`,
      {
        ...data,
      }
    );
...
export const getAllCMSPages = async (userId, search, page, pageSize) => {
  ...
  const url = new URL(
    `${process.env.NEXT_PUBLIC_DOMAIN_URL}${API_ENDPOINTS.GET_ALL_PAGE_CMS}/${userId}`
  );
  const params = {
    search: search || "",
    page: page || 1,
...
export const getPerticularCMSPage = async (userId, linkId) => {
  ...
  // Make the request if the cache is not available and store the data in the cache
  const url = new URL(
    `${process.env.NEXT_PUBLIC_DOMAIN_URL}${API_ENDPOINTS.GET_PAGE_CMS_DETAILS}/${linkId}?userId=${userId}`
  );
...
export const getPerticularCMSPageSectionsContent = async (
  ...
  // Make the request if the cache is not available and store the data in the cache
  const url = new URL(
    `${process.env.NEXT_PUBLIC_DOMAIN_URL}${API_ENDPOINTS.GET_PAGE_CMS_CONTENT_DETAILS}/${linkId}?userId=${userId}&lang=${lang}`
  );
...
export const updatePerticularCMSPage = async (
  ...
  try {
    const response = await axios.put(
      `${process.env.NEXT_PUBLIC_DOMAIN_URL}${API_ENDPOINTS.UPDATE_PAGE_CMS}/${linkId}?userId=${userId}`,
      {
        ...data,
      }
    );
    ...
    if (response.data.success && response.status === 200) {
      ...
      await Promise.all([
        ...
        deletePatternCache(
          `${cacheKeyNames.PAGE_CMS_FE_CONTENT_DETAILS}-${linkId || "linkId"}-*`
        ),
      ]);
      console.log(
        `updatePerticularCMSPage: ${cacheKeyNames.PAGE_CMS_DETAILS} ${cacheKeyNames.PAGES_CMS} ${cacheKeyNames.PAGE_CMS_CONTENT_DETAILS} ${cacheKeyNames.PAGE_CMS_FE_CONTENT_DETAILS} Cache deleted 🗑️`
      );
...
export const updatePerticularCMSPageSections = async (
  ...
  try {
    const response = await axios.patch(
      `${process.env.NEXT_PUBLIC_DOMAIN_URL}${API_ENDPOINTS.UPDATE_PAGE_CMS_CONTENT}/${linkId}?userId=${userId}&lang=${lang}`,
      {
        data,
      }
    );
    ...
    if (response.data.success && response.status === 200) {
      ...
      console.log(
        `updatePerticularCMSPageSections: ${cacheKeyNames.PAGE_CMS_CONTENT_DETAILS} ${cacheKeyNames.PAGE_CMS_FE_CONTENT_DETAILS} Cache deleted 🗑️`
      );

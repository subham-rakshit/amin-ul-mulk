...
export const createNewNewsArticle = async (data, userId) => {
  try {
    const response = await axios.post(
      `${process.env.NEXT_PUBLIC_DOMAIN_URL}${API_ENDPOINTS.CREATE_NEW_NEWS_ARTICLE}/${userId}`,
      {
        ...data,
      }
    );
    ...
    if (response.data.success && response.status === 201) {
      ...
      await deletePatternCache(`${cacheKeyNames.NEWS_ARTICLES_FE}-*`);
      console.log(
        `createNewNewsArticle: ${cacheKeyNames.NEWS_ARTICLES} ${cacheKeyNames.NEWS_ARTICLES_FE} Cache deleted 🗑️`
      );
...
export const getAllNewsArticles = async (
  ...
  const keyName = `${cacheKeyNames.NEWS_ARTICLES}-${search || "search"}-${page || 1}-${pageSize || 5}-${status || "status"}-${featured || "featured"}`;
  ...
  const url = new URL(
    `${process.env.NEXT_PUBLIC_DOMAIN_URL}${API_ENDPOINTS.GET_ALL_NEWS_ARTICLES}/${userId}`
  );
  const params = {
    search: search || "",
    page: page || 1,
...
export const getPerticularNewsArticle = async (userId, slug, targetId) => {
  ...
  // Make the request if the cache is not available and store the data in the cache
  const url = new URL(
    `${process.env.NEXT_PUBLIC_DOMAIN_URL}${API_ENDPOINTS.GET_NEWS_ARTICLE_DETAILS}/${targetId}?userId=${userId}`
  );
...
export const updatePerticularNewsArticle = async (
  ...
  try {
    const response = await axios.put(
      `${process.env.NEXT_PUBLIC_DOMAIN_URL}${API_ENDPOINTS.UPDATE_NEWS_ARTICLE}/${targetId}?userId=${userId}`,
      {
        ...data,
      }
    );
    ...
    if (response.data.success && response.status === 200) {
      ...
      await Promise.all([
        ...
        deletePatternCache(`${cacheKeyNames.NEWS_ARTICLES_FE}-*`),
        deletePatternCache(
          `${cacheKeyNames.NEWS_ARTICLE_DETAILS_FE}-${slug || "slug"}-*`
        ),
      ]);
      console.log(
        `updatePerticularNewsArticle: ${cacheKeyNames.NEWS_ARTICLE_DETAILS} ${cacheKeyNames.NEWS_ARTICLES} ${cacheKeyNames.NEWS_ARTICLES_FE} ${cacheKeyNames.NEWS_ARTICLE_DETAILS_FE} Cache deleted 🗑️`
      );
...
export const deletePerticularNewsArticle = async (userId, slug, targetId) => {
  ...
  const url = new URL(
    `${process.env.NEXT_PUBLIC_DOMAIN_URL}${API_ENDPOINTS.DELETE_NEWS_ARTICLE}/${targetId}?userId=${userId}`
  );
  ...
  try {
    ...
    if (response.data.success && response.status === 200) {
      ...
      console.log(
        `deletePerticularNewsArticle: ${cacheKeyNames.NEWS_ARTICLE_DETAILS} ${cacheKeyNames.NEWS_ARTICLES} ${cacheKeyNames.NEWS_ARTICLES_FE} ${cacheKeyNames.NEWS_ARTICLE_DETAILS_FE} Cache deleted 🗑️`
      );
...
export const togglePerticularNewsArticleActiveStatus = async (
  ...
  try {
    ...
    const response = await axios.patch(
      `${process.env.NEXT_PUBLIC_DOMAIN_URL}${API_ENDPOINTS.TOGGLE_NEWS_ARTICLE_ACTIVE_STATUS}/${targetId}?userId=${userId}`
    );
    ...
    if (response.data.success && response.status === 200) {
      ...
      console.log(
        `togglePerticularNewsArticleActiveStatus: ${cacheKeyNames.NEWS_ARTICLE_DETAILS} ${cacheKeyNames.NEWS_ARTICLES} ${cacheKeyNames.NEWS_ARTICLES_FE} ${cacheKeyNames.NEWS_ARTICLE_DETAILS} Cache deleted 🗑️`
      );
...
export const togglePerticularNewsArticleFeaturedStatus = async (
  ...
  try {
    // Make the patch request using fetch
    const response = await axios.patch(
      `${process.env.NEXT_PUBLIC_DOMAIN_URL}${API_ENDPOINTS.TOGGLE_NEWS_ARTICLE_FEATURED_STATUS}/${targetId}?userId=${userId}`
    );
    ...
    if (response.data.success && response.status === 200) {
      ...
      console.log(
        `togglePerticularNewsArticleFeaturedStatus: ${cacheKeyNames.NEWS_ARTICLE_DETAILS} ${cacheKeyNames.NEWS_ARTICLES} ${cacheKeyNames.NEWS_ARTICLES_FE} ${cacheKeyNames.NEWS_ARTICLE_DETAILS_FE} Cache deleted 🗑️`
      );

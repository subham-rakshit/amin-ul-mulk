...
export const createNewTestimonial = async (data, userId) => {
  try {
    const response = await axios.post(
      `${process.env.NEXT_PUBLIC_DOMAIN_URL}${API_ENDPOINTS.CREATE_NEW_TESTIMONIAL}/${userId}`,
      {
        ...data,
      }
    );
...
export const getAllTestimonials = async (
  ...
  const keyName = `${cacheKeyNames.TESTIMONIALS}-${search || "search"}-${page || 1}-${pageSize || 9}-${status || "status"}`;
  ...
  const url = new URL(
    `${process.env.NEXT_PUBLIC_DOMAIN_URL}${API_ENDPOINTS.GET_ALL_TESTIMONIALS}/${userId}`
  );
  const params = {
    search: search || "",
    page: page || 1,
...
export const getPerticularTestimonial = async (userId, targetId) => {
  ...
  // Make the request if the cache is not available and store the data in the cache
  const url = new URL(
    `${process.env.NEXT_PUBLIC_DOMAIN_URL}${API_ENDPOINTS.GET_TESTIMONIAL_DETAILS}/${targetId}?userId=${userId}`
  );
...
export const updatePerticularTestimonial = async (userId, targetId, data) => {
  try {
    const response = await axios.put(
      `${process.env.NEXT_PUBLIC_DOMAIN_URL}${API_ENDPOINTS.UPDATE_TESTIMONIAL}/${targetId}?userId=${userId}`,
      {
        ...data,
      }
    );
...
export const deletePerticularTestimonial = async (
  ...
  const url = new URL(
    `${process.env.NEXT_PUBLIC_DOMAIN_URL}${API_ENDPOINTS.DELETE_TESTIMONIAL}/${targetId}?userId=${userId}`
  );
...
export const togglePerticularTestimonialActiveStatus = async (
  ...
  try {
    ...
    const response = await axios.patch(
      `${process.env.NEXT_PUBLIC_DOMAIN_URL}${API_ENDPOINTS.TOGGLE_TESTIMONIAL_ACTIVE_STATUS}/${targetId}?userId=${userId}`
    );
